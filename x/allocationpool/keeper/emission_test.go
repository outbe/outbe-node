...
func (suite *KeeperTestHelper) TestCalculateExponentialBlockEmission() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("BlockHeight=%d", tc.blockHeight), func() {
			tokenStr, err := suite.App.AllocationPoolKeeper.CalculateExponentialBlockEmission(suite.Ctx, tc.blockHeight)
...
func (suite *KeeperTestHelper) TestGetEmission() {
	...
	emission, found := suite.App.AllocationPoolKeeper.GetTotalEmission(suite.Ctx)
